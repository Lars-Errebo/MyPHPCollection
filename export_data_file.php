...
function drawMenuSelectors($startdate, $enddate, $chainSelected, $chainArray, $statusSelected, $productClasses, $prodclassSelected, $csvColumns)
{
	...
	echo '<tr><td><div style="width:100px;"><strong>Udtræk efter</strong></div></td>';
...
$start_date = "";
$end_date = "";
$csvColumns = "startdato;policenummer;rec_id;kontoid;navn;adresse;model;serieno;koebspris;praemie;forhandler;loebetid;forsikringsstatus;";
...
$csvColumns .= "annulleringsdato;annulleringsgrund;forsikringstype;kategori;aktiveringsdato;dækningstype;kontant;betaltibutik;";
...
if (isset($_POST['sortby']))
{
	...
	else
	{
		...
		// Præ-indlæsning af annulleringstekster..
		...
		while(($d = mysql_fetch_assoc($query)) != FALSE)
		{
			...
			// Dï¿½kningstype til TopDK
			if (in_array("-Alle-",$_POST['colsel']) || in_array("dækningstype",$_POST['colsel']))
			...
			if (in_array("-Alle-",$_POST['colsel']) || in_array("betaltibutik",$_POST['colsel']))
				$res .= '"'.($d['paid'] == "" ? toDecimal($d['policy_price']) : toDecimal($d['paid'])).'";';//$d['paid']).'";'; //betaltibutik	//nogle gange returnerer denne et belï¿½b alla"299" nogle gange "267.00", punktummet forstyrer i CSV formatet.
...
stdHeader("Forsikring CSV Eksport - afgrænsning");
