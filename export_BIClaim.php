...
return floor((abs($diff->format('%a')/$f)+1));
}
...
$header  = "skadenr;kunde;policenummer;startdato;alder;praemie;salgsdato;forhandler;klasse;producent;model;produktkoebspris;gruppe;skadetype;";
...
$header .= "postnrby;fastnet;mobilnr;kontant;email;d�kningstype;policestatus;lobetid;kreditnota;risikoklasse";
	...
	else
	{
		...
		while( ($d = mysql_fetch_assoc($result)) != FALSE)
		{
			...
			{	$res .= '"'.$info['policy_number'].'";';
				$res .= '"'.$info['start_date'].'";';
				$res .= '"'.Days_diff($info['start_date'],$d['created'],'y').'";';	
			}	
			...
			if (in_array("-Alle-",$_POST['colsel']) || in_array("produktkoebspris",$_POST['colsel']))
				...
				$res .= '"'.Price_Group($prod['purchase_price']).'";';
...
stdHeader("Skade CSV Eksport - afgr�nsning");
