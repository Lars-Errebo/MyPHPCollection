...
function drawMenuSelectors($startdate, $enddate, $perdate, $chainSelected, $chainArray, $statusSelected, $productClasses, $prodclassSelected)
{
	echo '
		<link rel="stylesheet" type="text/css" href="' . POINTADMINURL . '/datepickr.css" />
          ...
          <table border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td><div style="width:60px;"><strong>Per dato</strong></div></td>
		    </tr>
	';
	...
	echo '<td>';
	echo '<input style="width:90px;" type="text" name="perDate" id="perDate" value="' . $perdate . '" />';
	echo '<script type="text/javascript" src="' . POINTADMINURL . '/datepickr.js"></script>
		  <script type="text/javascript">			
			         new datepickr(\'perDate\', {
				\'dateFormat\': \'d-m-Y\',
					weekdays: [\'s�ndag\', \'mandag\', \'tirsdag\', \'onsdag\', \'torsdag\', \'fredag\', \'l�rdag\'],
				    months: [\'januar\', \'februar\', \'marts\', \'april\', \'maj\', \'juni\', \'juli\', \'august\', \'september\', \'oktober\', \'november\', \'december\'],
	});</script>';	
...
if (isset($_POST['sortby']))
{
	...
	{
		$myDateTime = DateTime::createFromFormat('d-m-Y', $_POST['perDate']);
		$ymdPerDate = $myDateTime->format('Y-m-d');
		
		if (!empty($ymdStartDate))
			$end_date = "AND ".$sortbyfield." <= '".$ymdPerDate."' ";
	}
	...
	{	
		...
		$res = "";
		$res .= "startdato;slutdato;alder;policenummer;rec_id;kontoid;navn;adresse;model;serieno;koebspris;gruppe;praemie;forhandler;loebetid;forsikringsstatus;";
		...
		{		
			...
			$res .= '"'.DateFormat($d['end_date']).'";';
			
			$policy_end = min($d['end_date'],date('Y-m-d'));
			
			$res .= '"'.Days_diff($d['start_date'],$policy_date,'y').'";'; 
...
drawMenuSelectors(@$_POST['startDate'], @$_POST['endDate'], @$_POST['perDate'], @$_POST['shopchain'], $chainArray, @$_POST['statusselected'], $pcArray, @$_POST['productclass']);
